...
async def show_product_details(call: CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    product = await molten_api.get_product_details(callback_data["product_id"])
    product_description = f"""<b>{product.get("name")}</b>\nЦена: {product['meta']['display_price']['with_tax']['formatted']}\n<i>{product.get("description")}</i>"""

    image_link = await molten_api.get_product_main_image_link(product)
...
async def show_cart_items(call: CallbackQuery):
    ...
    cart_items = await molten_api.get_cart_items(call.from_user.id)

    cart_items_description = "\n".join(
        [f"""<b>{item.get("name")}</b>\nКоличество: {item['quantity']}""" for item in cart_items]
    )
    cart_items_description += f"\n\n<b>Итого: {await molten_api.get_cart_price(call.from_user.id)}</b>"
...
async def remove_item_from_cart(
    ...
    await molten_api.remove_item_from_cart(call.from_user.id, callback_data["item_id"])
...
async def receive_email(message: types.Message, state: FSMContext):
    try:
        await molten_api.create_customer(
    ...
    except molten_api.CreateCustomerException:
        await message.answer(
